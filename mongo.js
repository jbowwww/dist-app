...
module.exports = _.assign(mongoose, {
	globalPlugin(schema, options) {
		console.verbose(`global schema: schema=${inspect(schema)}`);

	// TODO: Add these only to the top-level models/schemas, not the embedded stats schema
		schema.add({
		...
		schema.pre('validate', function(next) {
			console.verbose(`store: ${inspect(_.keys(this))}`);
		...
		schema.methods.updateDocument = function updateDocument(update, pathPrefix = '') {
			console.debug(`updateDocument: ${update.path}`);
			_.keys(update).forEach(k => {
				...
				if (this.schema.path(pathPrefix + k).instance === 'Embedded') {
					...
					console.debug(`${pathPrefix + k}: Updating ${docVal} to ${updVal}`);
					this.set(pathPrefix + k, updVal);
				} else {
					console.debug(`${pathPrefix + k}: No update to ${docVal}`);
				}
			});
			return Q(this);
		};
		...
		// 		rawData.createdAt = rawData.updatedAt = new Date();
		...
		// 		output.write({ updateOne: { filter: { _id: id }, update: { $setOnInsert: { 'createdAt': new Date() }, $set: rawData }, upsert: true } }, '', innerCb);
		// 	} else {
		...
		schema.statics.findOrCreate = function  findOrCreate(query, file, cb) {
			...
			return Q.Promise((resolve, reject, notify) => {
				model.findOne(query, (err, r) => {
					...
					if (!r) {
						r = _.assign(model.create(file));	//, { type: file.type }
